...
class CallbackStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    //EVENTBRIDGE
    const dailyRule = new Rule(this, 'DailyTTLCleanupRule', {
      schedule: Schedule.rate({ days: 1 }),
      description: 'Daily trigger for TTL cleanup Lambda function'
    });
    
    dailyRule.addTarget(new LambdaFunction(getItemsDeleteByTtlFn));

		//-----------------------------------------------------
		// LAMBDA FUNCTIONS
		//-----------------------------------------------------

    const getItemsDeleteByTtlFnId = 'get-items-delete-by-ttl';
    const getItemsDeleteByTtlFn = this.createFunction(
      cbWriteFnId,
      connectInstanceArn,
      normalize(join(__dirname, 'lambda', getItemsDeleteByTtlFnId, 'index.js')),
      Runtime.NODEJS_18_X,
    ...
    const sentEmailFnId = 'sent-email';
    const sentEmailFn = this.createFunction(
      cbWriteFnId,
      connectInstanceArn,
      normalize(join(__dirname, 'lambda', sentEmailFnId, 'index.js')),
      Runtime.NODEJS_18_X,
      this.getRole(cbWriteFnId, [ dbWritePolicy ]),
      { TableName: callbacksTbl.tableName },
      [ '@aws-sdk/client-dynamodb' ]
